...
import { getTelegramFileUrl } from "~/utils/get-telegram-file";
import { useModal } from "./modal";

export const PicturePostCard = ({ post }: CardProps) => {
  //   console.log(post.images);

  const [img, mediumImg] = post.images?.reverse();
  ...
  return (
    <View>
      ...
      <View className="m-2 mx-4">
        <Text className="text-lg text-white">{post.description}</Text>
      </View>
    </View>
  );
